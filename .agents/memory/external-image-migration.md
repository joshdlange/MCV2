...
- **Cloudinary upload errors are plain objects, not Error instances** ({ message, http_code }): `String(error)` yields "[object Object]", which hides 404s from the permanent-failure detector and silently blocks all clearing. Always extract message + http_code explicitly (see toErrorMessage in the migration service) for any new Cloudinary error handling.
