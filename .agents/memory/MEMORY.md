...
- [Subscription truth](subscription-truth.md) — paying includes scheduled cancellations; expired subscriptions are not complimentary; decline recovery is distinct from canceled churn.
...
- [Atomic collection adds](atomic-collection-adds.md) — distinct-card limits and inserts share one per-user transaction; owned-card retries must remain no-ops at the limit.
