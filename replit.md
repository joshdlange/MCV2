...
- **Email Integration**: Two providers, split by flow (July 2026):
  - **Resend** (`RESEND_API_KEY`, from `no-reply@marvelcardvault.com`): password reset emails ONLY (plus admin Resend test endpoint `POST /api/admin/test-resend-email`). Service functions `sendResendEmail`/`sendPasswordResetEmail` in `server/services/emailService.ts`. Password reset flow: client `resetPassword()` → public `POST /api/auth/forgot-password` → Firebase Admin `generatePasswordResetLink` (token/expiry handled by Firebase) → branded `passwordResetTemplate` sent via Resend. Anti-enumeration: unknown emails get the same generic success response. Sends are logged to `email_logs` (template `password-reset`).
  - **Brevo SMTP** (`sendEmail` in same file): ALL other transactional email — welcome, monthly nudges/digest, marketplace notifications, coupon blasts, admin campaigns — plus Firebase-to-Brevo contact sync. No marketing sends via Resend.
  - `POST /api/test-email` (Brevo test) is now admin-only (was previously unauthenticated — fixed July 2026).
...
- **PC Binders**: User-created custom binders (`pc_binders` + `pc_binder_cards` tables) with name, optional description, and category (Character/Artist/Theme/Chase List/Other). Hold owned AND unowned cards — owned render normal with green "Owned" badge, unowned grayscale with amber "Chasing" badge. Progress = owned/total. Private-only (no sharing), never mutates user_collections. Caps: 50 binders/user, 500 cards/binder (no pagination). Routes: `/pc-binders` list + `/pc-binders/:id` detail; sidebar nav "PC Binders" (BookOpen icon). API under `/api/pc-binders`, all owner-scoped.
