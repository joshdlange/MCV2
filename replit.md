...
- **iOS App Store Compliance**: Native Apple In-App Purchase (StoreKit) for iOS subscriptions via `cordova-plugin-purchase`. Product ID: `MCV_Apple_Superhero`. Backend receipt verification at `POST /api/apple-iap/verify` validates with Apple's verifyReceipt API (production + sandbox fallback). `appleOriginalTransactionId` stored on user record. Android still uses Stripe via external browser. Sign in with Apple added for Guideline 4.8 compliance (iOS native only, uses Firebase OAuthProvider). Account deletion added for Guideline 5.1.1(v) compliance (DELETE /api/user/account, transactional cleanup of all user data).
