...
export default function AccountSettings() {
  ...
  // Hydrate form with saved profile values once loaded
  useEffect(() => {
    if (!userProfile) return;
    let address = { street: '', city: '', state: '', postalCode: '', country: '' };
    const raw = (userProfile as any).shippingAddressJson ?? (userProfile as any).address ?? (userProfile as any).shippingAddress;
    if (raw) {
      try {
        const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
        address = {
          street: parsed.street1 || parsed.street || '',
          city: parsed.city || '',
          state: parsed.state || '',
  ...
  const togglePrivacy = (key: keyof typeof profileData.privacySettings) => {
    const next = {
      ...profileData,
      privacySettings: { ...profileData.privacySettings, [key]: !profileData.privacySettings[key] }
    };
    setProfileData(next);
  ...
  const toggleNotification = (key: keyof typeof profileData.notifications) => {
    const next = {
      ...profileData,
      notifications: { ...profileData.notifications, [key]: !profileData.notifications[key] }
    };
    setProfileData(next);
    savePreferences(next);
  };
                    ...
                    <div>
                      ...
                      {userProfile?.stripeCustomerId ? (
                        ...
                        <p className="text-sm text-gray-500">
                          If you subscribed on iPhone or iPad, manage or cancel your subscription in the App Store (Settings → your name → Subscriptions). Otherwise, your access was granted directly and there's nothing to manage here.
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="bg-white text-black border-gray-300 hover:bg-gray-100 rounded-full"
                      onClick={() => togglePrivacy('showCollection')}
                    >
                  ...
                  <div className="flex items-start justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <div className="flex items-start gap-2 pr-3">
                      <ImageIcon className="w-4 h-4 text-gray-500 mt-0.5" />
                      <div>
                        <span className="text-sm text-gray-700">Show my name on images I contribute</span>
                        <p className="text-xs text-gray-500 mt-0.5">
                          When on, cards you upload images for credit you as the contributor. Turn off to stay anonymous.
                        </p>
                      </div>
                    </div>
                    <Button
                      ...
                      onClick={() => togglePrivacy('showImageAttribution')}
                    >
                      {profileData.privacySettings.showImageAttribution ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                    </Button>
                  </div>
                </div>
              </div>
