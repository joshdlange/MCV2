...
import subsetData from './data/toppsChromeSapphire2026.json';

/**
 * One-time idempotent seed: 2026 Topps Chrome Sapphire Edition.
 *
 * Creates (only if missing — safe to run on every startup, dev and prod):
 *  - The master set (matched by slug), with the sealed box image
 *  - 29 subsets (Base + 28 parallels/inserts/autographs)
 *  - 1,738 cards total
 *
 * Data source: server/seeds/data/toppsChromeSapphire2026.json, generated from
 * the user's spreadsheet with these cleanups: stripped CRLF, normalized
 * "HIDDEN GEMS" → "Hidden Gems" and "- RED" → "- Red", and removed 24 exact
 * duplicate rows (identical number + name repeated in the facsimile autograph
 * subsets).
 */

const MAIN_SET_SLUG = '2026-topps-chrome-sapphire-edition';
const MAIN_SET_NAME = '2026 Topps Chrome Sapphire Edition';
const YEAR = 2026;
const SET_IMAGE_URL = 'https://res.cloudinary.com/dgu7hjfvn/image/upload/v1786381481/set_images/yhr4ds9fbrw0kvxzy5s1.jpg';
...
export async function seedToppsChromeSapphire2026(): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`SELECT pg_advisory_xact_lock(hashtext('seed-topps-chrome-sapphire-2026'))`);
    ...
    if (!mainSet) {
      ...
      console.log('[Chrome Sapphire Seed] Created master set');
    ...
    console.log(`[Chrome Sapphire Seed] Subsets ensured: ${SUBSETS.length} (${createdSubsets} newly populated), cards inserted: ${insertedCards}`);
  });

  console.log('[Chrome Sapphire Seed] ✅ 2026 Topps Chrome Sapphire Edition is in place');
}
