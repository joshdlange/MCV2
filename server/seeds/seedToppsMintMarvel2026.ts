...
async function ensureSubset(tx: Tx, mainSetId: number, name: string, isInsertSubset: boolean): Promise<number> {
  ...
  const [bySlug] = await tx.select({ id: cardSets.id, mainSetId: cardSets.mainSetId })
    .from(cardSets).where(eq(cardSets.slug, slug));
  if (bySlug) {
    if (bySlug.mainSetId !== mainSetId) {
      throw new Error(`[Topps Mint Seed] Slug collision: "${slug}" already belongs to main set ${bySlug.mainSetId}, expected ${mainSetId} — aborting`);
    }
    return bySlug.id;
  }
...
export async function seedToppsMintMarvel2026(): Promise<void> {
  ...
  const [check] = await db.select({
    ...
    cardCount: sql<number>`count(distinct (${cards.setId}, ${cards.cardNumber}))::int`,
  })
    .from(cardSets)
