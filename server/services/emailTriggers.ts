...
export async function onBadgeUnlocked(user: User, badgeInfo: BadgeInfo): Promise<void> {
  try {
    ...
    await sendEmail(user.email, `New Badge Unlocked: ${badgeInfo.name}! 🏆`, html);
  } catch (error) {
...
export async function onCardImageApproved(user: User, card: Card): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'Card Image Approved! ✅', html);
  } catch (error) {
...
export async function onNewSetAnnouncement(user: User, setInfo: SetInfo): Promise<void> {
  try {
    ...
    await sendEmail(user.email, `New Set Alert: ${setInfo.name}! 🚨`, html);
  } catch (error) {
...
export async function onAddFirstCardNudge(user: User): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'Ready to Start Your Collection? 📚', html);
  } catch (error) {
...
export async function onFinishSetupNudge(user: User): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'Complete Your Profile 👤', html);
  } catch (error) {
...
export async function onInactivityReminder(user: User): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'We Miss You at Marvel Card Vault! 💔', html);
  } catch (error) {
...
export async function onCollectionMilestone(user: User, milestone: Milestone): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'Collection Milestone Achieved! 🎊', html);
  } catch (error) {
...
export async function onWeeklyDigest(user: User, sets: NewSet[]): Promise<void> {
  try {
    ...
    await sendEmail(user.email, 'Your Weekly Marvel Card Vault Digest 📬', html);
  } catch (error) {
